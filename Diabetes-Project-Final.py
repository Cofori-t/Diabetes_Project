...
I created a Model that could be used to predict the likelihood of Diabetes in patients based on their Medical history and Demographic details.

Dependent Var: 

:blue[Gender, Age, Hypertension(Blood-Pressure), Heart Disease, Body Mass Index (BMI), Smoking history, HbA1c level, and Blood Glucose Level]

Target Var:

:blue[Diabetes. (Binary outcome)]
...
with st.container():
    ...
    st.sidebar.subheader(" :blue[1. Complete: (Med. Hist./Demog) questionnaire,  2. Click on: Submit Answers]")
    st.sidebar.write("##")
